...
const schema = {
  ...
  querystring: {
    ...
    bounds: {
      type: 'string',
      pattern: '^((-?\\d+\\.?\\d+)(,-?\\d+\\.?\\d+){2,3})',
      description: 'sssssssssssssOptionally limit output to features that intersect bounding box. Can be expressed as a bounding box (sw.lng, sw.lat, ne.lng, ne.lat) or a Z/X/Y tile (0,0,0).'
    },
    limit: {
      type: 'integer',
